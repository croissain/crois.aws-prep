...
export function Practice({ questions, exam, savedOnly = false }: { questions: Question[]; exam: Exam; savedOnly?: boolean }) {
  ...
  return <section className="mx-auto max-w-5xl px-5 pb-24 pt-12 lg:px-8 lg:pt-16">
    ...
    <div className="mt-7 space-y-4">{visible.map((q, i) => <QuestionCard key={q.id} q={q} examCode={exam.code} number={(page-1)*pageSize+i+1} selected={answers[q.id] || []} onSelect={selected => setAnswers(current => ({ ...current, [q.id]: selected }))} bookmarked={bookmarks.has(q.id)} onBookmark={() => setBookmarks(current => { const next = new Set(current); next.has(q.id) ? next.delete(q.id) : next.add(q.id); saveBookmarks(next); return next; })} revealed={open.has(q.id)} toggle={() => setOpen(s => { const n = new Set(s); n.has(q.id) ? n.delete(q.id) : n.add(q.id); return n; })}/>)}</div>
...
function QuestionCard({ q, examCode, number, selected, onSelect, bookmarked, onBookmark, revealed, toggle }: { q: Question; examCode: string; number: number; selected: string[]; onSelect: (selected: string[]) => void; bookmarked: boolean; onBookmark: () => void; revealed: boolean; toggle: () => void }) {
